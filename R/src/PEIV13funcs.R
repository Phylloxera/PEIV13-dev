...
PEIV13varcheck <- function() {if (is.integer(reversetrimLeft) == F) stop(
  ...
  message(
    ...
    "(points)\nand the error rates drop with increased quality as expected you",
    " can have\nconfidence in the error model.  Example plots that might justi",
    "fy enforcing\nmonotonicity in the fitted error model are here:\nhttps://g",
    "ithub.com/benjjneb/dada2/issues/1156.  During merging, sequences are\nonl",
    "y output if the forward and reverse reads overlap by at least 12 bases, a",
...
cat("Chimera removal completed in", paste0(elapsed_time, "!\n")); message(
  ...
  " ", round(dim(schim)[2] / dim(st.mr)[2] * 100, 2), "% of merged sequence va",
  "riants remain\nin the sequence table with chimeras removed.  Most of your r",
  "aads should remain\nafter chimera removal (it is not uncommon for a majorit",
  "y of sequence variants to\nbe removed though).  If most of your reads were ",
  "removed as chimeric, upstream\nprocessing may need to be revisited.  In alm",
  "ost all cases this is caused by\nunremoved primers interfering with chimera",
